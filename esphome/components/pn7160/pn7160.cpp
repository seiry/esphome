...
bool PN7160::reset_core_(bool reset_config, bool power) {
  ...
  // this->wkup_req_pin_->digital_write(true);
  ...
  if (!this->write_and_read_(NCI_CORE_GID, NCI_CORE_RESET_OID, write_data, response, 50)) {
...
bool PN7160::init_core_(std::vector<uint8_t> &response, PN7160State next_state) {
...
bool PN7160::set_mode_(PN7160Mode mode) {
...
bool PN7160::start_discovery_(PN7160Mode mode) {
  std::vector<uint8_t> response;
...
void PN7160::loop() {
  switch (this->state_) {
    case PN7160State::SET_MODE:
      this->set_mode_(PN7160Mode::READ_WRITE);
      return;
    case PN7160State::DISCOVERY:
      this->start_discovery_(PN7160Mode::READ_WRITE);
      return;
    ...
    if (gid == RF_GID) {
      switch (oid) {
        case RF_INTF_ACTIVATED_OID: {
          ...
          ESP_LOGD(TAG, "Detected interface: 0x%.2X, protocol: 0x%.2X, tech: 0x%.2X", interface, protocol, mode_tech);
...
bool PN7160::write_and_read_(uint8_t gid, uint8_t oid, const std::vector<uint8_t> &data, std::vector<uint8_t> &response,
                             uint16_t timeout, bool warn) {
  return this->write_and_read_(gid, oid, data.data(), data.size(), response, timeout, warn);
}

bool PN7160::write_and_read_(uint8_t gid, uint8_t oid, const uint8_t *data, const uint8_t len,
  ...
  if ((response[0] & GID_MASK) != gid || (response[1] & OID_MASK) != oid) {
    ESP_LOGE(TAG, "Incorrect response: 0x%.2x 0x%.2x", response[0], response[1]);
    return false;
  }
...
bool PN7160::wait_for_irq_(uint16_t timeout, bool warn) {
