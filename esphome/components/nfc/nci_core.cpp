...
const uint8_t NciMessage::get_message_type() const {
  return this->nci_message_[NCI_PKT_MT_GID_OFFSET] & NCI_PKT_MT_MASK;
}

const uint8_t NciMessage::get_gid() const { return this->nci_message_[NCI_PKT_MT_GID_OFFSET] & NCI_PKT_GID_MASK; }

const uint8_t NciMessage::get_oid() const { return this->nci_message_[NCI_PKT_OID_OFFSET] & NCI_PKT_OID_MASK; }
...
const bool NciMessage::has_payload() const { return this->nci_message_.size() > NCI_PKT_HEADER_SIZE; }
...
void NciMessage::set_message(const uint8_t message_type, const std::vector<uint8_t> &payload) {
  this->nci_message_.resize(NCI_PKT_HEADER_SIZE);
  this->nci_message_[NCI_PKT_LENGTH_OFFSET] = payload.size();
...
void NciMessage::set_payload(const std::vector<uint8_t> &payload) {
  std::vector<uint8_t> message(this->nci_message_.begin(), this->nci_message_.begin() + NCI_PKT_HEADER_SIZE);

  message.insert(message.end(), payload.begin(), payload.end());
  message[NCI_PKT_LENGTH_OFFSET] = payload.size();
